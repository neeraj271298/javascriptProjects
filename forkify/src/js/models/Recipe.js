...
export default class Recipe {
    ...
    async getRecipe() {
        try {
            const res = await axios(`${proxy}http://food2fork.com/api/get?key=${key1}&rId=${this.id}`);
    ...
    parseIngredients() {
        ...
        const newIngredients = this.ingredients.map(el => {
            ...
            const unitIndex = arrIng.findIndex(el1 => unitsShort.includes(el1));
            let objIng;
            if(unitIndex > -1){
                // contain unit in ing.
                ...
                // contain unit with first element is number
                objIng = {
